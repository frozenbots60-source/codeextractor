...
class CodeDisplayDashboard:
    def __init__(self):
        ...
        # Pointing to the local websocket server we are running
        self.broadcast_ws_url = "ws://127.0.0.1:8080/ws" 
        self.broadcast_ws = None
    ...
    def connect_broadcast_ws(self):
        """Connect to the local broadcast WebSocket"""
        try:
            # Check if we need to connect or reconnect
            if self.broadcast_ws is None or not self.broadcast_ws.connected:
                # websocket.enableTrace(True) # Uncomment for debugging
                self.broadcast_ws = websocket.WebSocket()
                self.broadcast_ws.connect(self.broadcast_ws_url, timeout=5)
                logging.info(f"[BROADCAST] Connected to {self.broadcast_ws_url}")
        except Exception as e:
            logging.error(f"[BROADCAST] Connection failed: {e}")
            self.broadcast_ws = None

    def fast_broadcast(self, code):
        """Directly sends the code over WebSocket for maximum speed"""
        payload = {
            "type": "stake_bonus_code",
            "code": code,
            "source": "fast_listener"
        }
        json_payload = json.dumps(payload)
        
        try:
            # Ensure we are connected
            self.connect_broadcast_ws()
            
            if self.broadcast_ws and self.broadcast_ws.connected:
                start = time.time()
                self.broadcast_ws.send(json_payload)
                ms = round((time.time() - start) * 1000, 2)
                logging.info(f"[FAST-SEND] Code '{code}' broadcasted in {ms}ms")
            else:
                logging.error("[FAST-SEND] Failed: WebSocket not connected")
                
        except Exception as e:
            logging.error(f"[FAST-SEND] Error broadcasting: {e}")
    ...
    def handle_socket_message(self, data):
        ...
        if data.get('type') == 'sub_code_v2':
            ...
            # FAST BROADCAST: Use a thread to avoid blocking the listener loop
            # We fire and forget to process the next incoming message ASAP
            threading.Thread(target=self.fast_broadcast, args=(code,), daemon=True).start()
    ...
    def connect_to_server(self):
        ...
        try:
            ...
            # Also init the broadcast socket
            self.connect_broadcast_ws()
    ...
    def start_heartbeat(self):
        ...
        def heartbeat():
            while self.running:
                ...
                # Also keep alive our broadcast socket
                try:
                    if self.broadcast_ws and self.broadcast_ws.connected:
                         self.broadcast_ws.ping()
                except:
                     pass
